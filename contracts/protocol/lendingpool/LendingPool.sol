...
contract LendingPool is VersionedInitializable, ILendingPool, LendingPoolStorage {
   ...
   * @param amount 要存入的数量
   * @param onBehalfOf 将接收 aToken 的地址，如果用户希望在自己的钱包上接收 aToken，则该地址与 msg.sender 相同；
   * 如果 aToken 的受益人是另一个钱包，则为不同的地址
   * @param referralCode 用于注册发起该操作的集成商的代码，用于潜在奖励。.
   *   如果该操作由用户直接执行，没有任何中间人，则为 0
   **/
  function deposit(
    address asset,
    uint256 amount,
